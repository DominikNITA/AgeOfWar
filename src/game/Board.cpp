...
void Board::moveUnitForward(IBaseUnit *unit, int count) {
    ...
    if (_boardData[newIndex] == nullptr) {
        ...
        p_gameLogger->log("Unit" + ConsoleHelper::getColorString(unit->getOwner()->getColorCode()) +
                                  unit->print() + ConsoleHelper::getColorString(RESET) + " from "+ ConsoleHelper::getColorString(BLUE) +std::to_string(unitPosition) + ConsoleHelper::getColorString(RESET) + " to "+ ConsoleHelper::getColorString(BLUE)+std::to_string(newIndex));
    }
}
